...
Controller / telemetry image for this run: `jumpstarter-controller:0.10.0-dev-44-g0f6b0970`. Exporter venv: `jumpstarter 0.10.0.dev44+g0f6b09704` on `jep-0013-phase3-demo` (`ba5be3e2`), which already registers MetricsStream with `identity=exporter_name` (needed after `#1058`).
...
This lab used asciinema 3 with a second client at the same size as the existing attach (`104x55`, tmux `window-size latest`) so the layout did not shrink:

```bash
TERM=xterm-256color asciinema rec --headless --window-size 104x55 \
  --idle-time-limit 3 \
...
1. Exporter came **Available**; telemetry attached; **MetricsStream registered** on the first `jmp run`.
...
Lease **`01a06412-2e27-72d1-844a-40e612af04c3`** (`p16v` → `sidekick-t450s`) was released at the end of the recorded run.
...
Local exporter `/metrics` was scraped on the sidekick (`127.0.0.1:45793` for this run). Telemetry reverse-scrape was `GET /metrics` on `svc/jumpstarter-telemetry` via `oc port-forward` (stopped after each scrape). Local series now label `exporter="sidekick-t450s"` via Session `_session_labels()`.

| When | Local exporter `:45793` | Telemetry reverse-scrape |
|------|-------------------------|--------------------------|
...
| After `start` / `on` / `wake` | `operations_*` + duration histograms + `lease_id` **exemplars**, `active_sessions=1` | snapshot **omitted**; `jumpstarter_metrics_parse_errors_total{exporter="sidekick-t450s"}` **5**, then **6** on the wrap scrape; `scrape_timeouts=0`, `dropped=0` |

The stream stays up (`scrape_timeouts` never incremented). After the first operation series with OpenMetrics exemplars (`# {lease_id="01a06412-…"} …`), telemetry `parseMetricFamilies` fails. The snapshot is still skipped so one exporter cannot 500 `/metrics`, but the failure is no longer silent: telemetry logs **error** `exporter metrics snapshot omitted` (exporter name + parse err) and increments `jumpstarter_metrics_parse_errors_total{exporter}`.

Parse-error counters on this telemetry pod were already **4** from prior lab scrapes; this run added two more (**5** after the first post-op scrape, **6** on wrap).
...
PushLogs carried the lease-correlated counts during the reverse-scrape gap:

- `operation`: `start`, `on`, `wake`, `get_host`, `get_port`, `get_url`
- `result`: `success`
- `driver_type`: `storage`, `network`, `power`, `other` (WoL)
- correlation: `lease=01a06412-2e27-72d1-844a-40e612af04c3`, `exporter=sidekick-t450s`, `namespace=jumpstarter-lab`
...
The file is asciicast v3, `104x55`, about 4.4 minutes of wall time (idle capped at 3s on play, ~1.5 minutes).
